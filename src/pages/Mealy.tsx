...
const MealyPage = () => {
  ...
  function InputsCallback(
    ...
    Alphabet.forEach((letter) => {
      NewColumns.push({
        title: `${letter} Gelirse Durum`,
        dataIndex: `${letter}_new`,
        key: `${letter}_1`,
      });
      NewColumns.push({
        title: `${letter} Gelirse Çıktı`,
        dataIndex: `${letter}_output`,
        key: `${letter}_2`,
      });
    });
    ...
    Situations.forEach((situation) => {
      ...
      Alphabet.forEach((letter) => {
        Row[`${letter}_new`] = situationSelect((data: string) => {
          ...
          temp[situation as any][`${letter}_new`] = data;
          TransactionTable.current = temp;
        });

        Row[`${letter}_output`] = outputSelect((data: any) => {
          ...
          temp[situation as any][`${letter}_output`] = data;
          TransactionTable.current = temp;
        });
      });
  ...
  function CheckCurrentTransactionTable() {
    ...
    for (let i = 0; i < Situations.length; i++) {
      ...
      for (let j = 0; j < Alphabet.length; j++) {
        ...
        if (
          !CurrentTransactionTable[situation][`${letter}_new`] ||
          !CurrentTransactionTable[situation][`${letter}_output`]
        )
          return error();
      }
    }
  ...
  function RunMachine() {
    ...
    word.forEach((letter) => {
      let output = CurrentSituationTable[situation][`${letter}_output`];
      situation = CurrentSituationTable[situation][`${letter}_new`];
